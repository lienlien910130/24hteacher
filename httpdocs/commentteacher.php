...
function cha($page,$order,$sort) {
  $.get("my_fi.php?page="+$page+"&&order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function chaa($order,$sort) {
  $.get("my_fi.php?order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
...
function cancel() {
  location.href="commentteacher.php";
}
</script>