 ...
 $pdo = DB_CONNECT();
   ...
   $sql = "SELECT * FROM parents WHERE types = 0 ORDER BY addtime DESC LIMIT $start_from, ".$results_per_page;
   $rs =$pdo ->query($sql);
   $x = 1;
   ?>
   <div class ='row'>
   ...
   <?php 
    $sql = "SELECT count(*) AS n FROM parents  WHERE types = 0";
    $rs = $pdo->query($sql);
       foreach ($rs as $key => $row) {
         $n = $row["n"];
         $total_pages = ceil( $n / $results_per_page);
         for ($i=1; $i<=$total_pages; $i++) { 
            echo "<a href='javascript:cha2(".$i.")' style='padding-left:10px;padding-right:10px'";
