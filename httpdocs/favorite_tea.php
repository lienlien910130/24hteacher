...
function cha($page,$order,$sort) {
  $.get("my_ei.php?page="+$page+"&&order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function chaa($order,$sort) {
  $.get("my_ei.php?order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
...
function deletefav($id){
  $.ajax({
            ...
            success: function(data,textStatus, jqXHR){
                 if(data.status == "1"){
               alert("移除成功");
               location.href="favorite_tea.php";
               die();
              }else{
               alert("刪除失敗，請詢問客服人員");
               location.href="favorite_tea.php";
