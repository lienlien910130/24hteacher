<?php 
  ...
  $o_old = $order;
  $s_old = $sort;
  $sql = "SELECT c.id as caseid , c.numbers as numbers , c.uid as cuid , c.name as name, c.phone as phone , c.addtime as addtime,c.lastedit as lastedit , c.applicants as applicants,f.id as fid,sc.obj as obj , sc.sub as sub , sc.area as area  FROM cases  c LEFT JOIN favorite  f on c.id = f.tid LEFT JOIN search_case sc on sc.caseid = c.id  WHERE f.types= 1 AND f.uid='".$uid."' ORDER BY $order $sort LIMIT $start_from,".$results_per_page;
...
<table class="table table-bordered table-hover col-lg-12 col-md-12 col-xs-12 col-sm-12">
            <thead>
              <tr class="th_tr">
                <td class="col-lg-2 col-md-2 col-xs-3 col-sm-3">
                ...
                <td class="col-lg-2 col-md-2 col-xs-1 col-sm-1">
                <!-- <a href="javascript:chaa('sendtime','<?php echo $sort?>')">最低時薪
                  <?php 
            ...
            <tbody>
               ...
               $cases = getCase();
                ...
                <tr class="changedtr<?php echo $row["fid"]?>">
                  ...
                  <a href="secase.php?id=<?php echo $row["caseid"]?>" target="_blank"><?php echo $row["numbers"]?></a>
                  </td>
                  <td class="col-lg-2 col-md-2 col-xs-2 col-sm-2 tabletd">
                  <?php echo $cases[$row["caseid"]][4];?>
                  </td>
                 
                  <td  class="col-lg-2 col-md-2 col-xs-2 col-sm-2 tabletd">
                  <?php echo $cases[$row["caseid"]][19];?>    
                  </td>
                  <td  class="col-lg-2 col-md-2 col-xs-2 col-sm-2 tabletd">
                  <?php echo $cases[$row["caseid"]][20];?>   
                  </td>
                  <td  class="col-lg-2 col-md-2 col-xs-1 col-sm-1 tabletd">
                  <?php echo $cases[$row["caseid"]][15];?>   
                  </td>
          ...
          <?php 
            $sql1 = "SELECT count(*) AS n FROM favorite WHERE uid='".$uid."' AND types = 1 ";
            $rs1 = $pdo->query($sql1);
