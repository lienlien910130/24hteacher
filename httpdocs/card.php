...
function cha($page,$order,$sort) {
  $.get("m_twel.php?page="+$page+"&&order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function chaa($order,$sort) {
  $.get("m_twel.php?order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function edit($id) {
  $.get("editman_card.php?id="+$id,function(data)
    {
...
function cancel() {
 ...
 function deletecard($id){
  var r = confirm("連同該履歷之此證照紀錄也會被刪除，刪除後無法再恢復記錄，確定刪除該項資料?");
  if (r == true) {
       $.ajax({
        type: "POST",
        url: 'deletecard.php',
