...
function cha($page,$order,$sort) {
  $.get("my_ten.php?page="+$page+"&&order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function chaa($order,$sort) {
  $.get("my_ten.php?order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
...
function cancelForm(){
       location.href = "commentcase.php";
     }
 function checkcoForm(){ 
      var frm = document.forms["commentForm"];
      re= /select|update|delete|exec|count|'|"|=|;|>|<|%/i;
      
      if(frm.commenttext.value == "" || frm.num.value== 0){
        alert("欄位不可空白");
      }else if(re.test(frm.commenttext.value.toLowerCase())){
         alert("欄位不可輸入無效字元");
      }
      else{
        frm.submit();
