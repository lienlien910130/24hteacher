...
function cha($page,$order,$sort) {
  $.get("my_el.php?page="+$page+"&&order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function chaa($order,$sort) {
  $.get("my_el.php?order="+$order+"&&sort="+$sort,function(data)
  {
      $("#person").html(data);
  });
}
function changeaccept($id,$status){
    ...
    $.ajax({
          ...
          success: function(data){
            alert("修改成功!");
            location.href="casedeal_tea.php";
          }
        });
     }
</script>